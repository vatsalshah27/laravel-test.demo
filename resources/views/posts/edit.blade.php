            ...
            <div class="mt-5 md:mt-0 md:col-span-2">
                <form method="post" action="{{ route('posts.update', $post->id) }}" enctype="multipart/form-data">
                    ...
                    <div class="shadow overflow-hidden sm:rounded-md">
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            <label for="title" class="block font-medium text-sm text-gray-700">Auto Generated Slug </label>
                            {{ $post->slug }}
                        </div>

                        <div class="px-4 py-5 bg-white sm:p-6">
                            <label for="description" class="block font-medium text-sm text-gray-700">Description <span class="text-red-600">*</span></label>
                            <textarea name="description" id="description" class="form-input rounded-md shadow-sm mt-1 block w-full" >{{ old('description', $post->description) }}</textarea>
                            @error('description')
                                <p class="text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <input type="date" name="published_at" id="published_at" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                   value="{{ old('published_at', $post->published_at) }}" placeholder="dd-mm-yyyy"  required/>
                            @error('published_at')
                                <p class="text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
